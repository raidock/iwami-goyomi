...
HELD_HEADS = [
    ...
    # に部分一致する（HELD_HEADS は `any(k in name ...)` で部分一致するため）。
    # 利益ゼロで危険だけがある。
]
...
# 「日」が抜けて「10月12（日）」と書かれることもある（邑南町観光協会で実例）。
# 「月DD」の直後が「（曜日）」なら、「日」が無くても日付として読む。
# **曜日1文字に絞ること。** 素朴な開き括弧だけを条件にすると、日付と無関係な
# 括弧書きまで拾う危険がある（実データでは踏んでいないが、絞める理由がある）。
# 公開162件で測って、この形は3件（うち1件は構造化された期間欄が別に勝つので
# 無変化）。誤爆0件・「8月2（土）3（日）」のような並びも見当たらなかった
# （2026-08-07）。
_DATE = re.compile(
...
#
# 全角数字＋「．」（「１．開催日時」）も同じ扱いに含める。大田市「しまねふるさと
# フェア２０２７」が、書き手が手で振った番号付き平文段落でこの形を使っていた
# （「開催日時」自体は既に登録済みの見出し語で、番号の記号が拾えていなかった
# だけ）。公開162件で測って、当たったのはこの1件のみ・誤爆は0件
# （2026-08-08。measurements/2026-08-08-unstructured-headings.md）。
#
# **半角の「1.」は入れない。** 公開162件で測ったところ出現は3件のみで、
# いずれも本文の箇条書き（体験内容の種類）であり見出し語には一致しなかった。
# 効果が無いうえ、半角ピリオドは英数字の本文により広く出るので危険度が高い。
_MARK_RE = re.compile(r"(?:[〇◯○●◆◇■□▲△▼▽★☆]|[０-９]+．)")
...
def extract_dates(html: str, ref: Optional[date] = None,
    ...
    if not got.date_start:
        ...
        # と**サイト側が年を打ち間違えて**いた。終わりだけ捨てると
        # 開催日が 7/14 として残り、**過ぎた日付になるので畳まれて消える。**
        # 日付が無いほうがまだよい（「日程は詳細ページで」と出て、リンクは生きる）。
        #
        # 形式が正しくても中身が正しいとは限らない（設計判断14）。
        # 逆転しているという**それ自体が、その欄を信じてはいけない証拠**。
        if len(declared) > 1 and declared[-1] < declared[0]:
            print(f"  [warn] 期間の宣言が逆さまなので使いません: "
                  f"{declared[0]} 〜 {declared[-1]}（{label[:24]}）")
            declared = []
        if declared:
