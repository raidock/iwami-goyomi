...
# タイトル頭の【カテゴリ】。1つとは限らない（【要申込】【8月】のように続く）
_BR = r"^\s*(?:[【〔\[][^】〕\]]{1,20}[】〕\]]\s*)+"
...
_HELD_RE = [
    ...
    # **【…】が前に付くだけの同じ型。** 津和野町観光協会はこれで書く。
    #   【津和野盆踊り】8/15　409回目の夏　殿町盆踊り開催のお知らせ
    #   【津和野町日本遺産センター】6/2～7/31「鷺舞企画展」開催
    # 実データ153件で測って、当たるのは津和野の6件だけ・全部正しい（2026-08-01）。
    # 【】そのものは24件あるが、日付が続くのはこの型に限られる。
    # **「まで」が続くときは開催日にしない**（それは締切）。
    re.compile(_BR + r"(?:(?P<y>\d{4})\s*年\s*)?(?P<m>\d{1,2})\s*月\s*(?P<d>\d{1,2})\s*日"
                     r"(?!\s*(?:まで|迄))"),
    # 続きが数字でも切らないこと。「【津和野盆踊り】8/15　409回目の夏」で
    # 409 に引っかかって取りこぼした。年つき（2026/8/1）は先頭の \d{1,2} が
    # 4桁に当たらないので、そもそも一致しない
    # `(?!\d)` が要る。**これが無いと後ろ向き言明を後戻りで回避される** —
    # 「8/31まで」は d を「3」だけに縮めれば「1まで」となり「まで」の
    # 言明をすり抜けて 8/3 になっていた
    re.compile(_BR + r"(?P<m>\d{1,2})\s*/\s*(?P<d>\d{1,2})(?!\d)(?!\s*/)"
                     r"(?!\s*(?:まで|迄))"),
    # **【…】の後ろではなく、中にスラッシュ日付がある型。** 川本町観光協会はこれで書く。
    #   【12/20開催】島根フィルティーズ　ファン感謝祭！！
    #   【11/14,15開催】弓ヶ峯八幡宮「秋の例大祭」
    # 「開催」を直後に要求するので、「【2/9更新】」のような更新日には当たらない
    # （実データで確認ずみ）。34件で測って、他の情報源には無い（川本町観光協会のみ）。
    re.compile(r"[【〔\[]\s*(?P<m>\d{1,2})\s*/\s*(?P<d>\d{1,2})(?:[,、]\s*\d{1,2})?"
               r"(?:\s*[（(][^）)]{0,3}[）)])?\s*開催"),
]
_DEADLINE_RE = [
